  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 py-8 pb-16">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="text-[10px] font-medium uppercase tracking-[0.18em] text-muted-foreground mb-1">Funds</div>
          <h1 className="text-[22px] font-semibold tracking-tight text-foreground">Wallet & Transactions</h1>
        </div>
      </div>
        ...
        {[
          ...
          <div key={i} className={`p-5 ${CARD}`}>
            ...
            {bl ? <Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /> : (
              <>
                <div className="text-[20px] font-semibold tracking-tight tabular-nums text-foreground mb-1">{item.value}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
        ...
        <div className={CARD}>
          ...
          <div className="p-6">
            {/* Pill toggle */}
            <div className="flex gap-1 bg-[#F2F3F5] p-1 rounded-full mb-6">
              <button onClick={() => setMode(mode === "deposit" ? "idle" : "deposit")}
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-[11px] font-medium rounded-full transition-all
                  ${mode === "deposit"
            ...
            {mode !== "idle" ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="flex rounded-xl overflow-hidden border border-[#E6E8EB] focus-within:border-[#0d1520] transition-colors">
                    ...
                    <input type="number" min="1" step="0.01" value={amount}
                      ...
                      placeholder="0.00"
                      className="flex-1 bg-white text-foreground text-[13px] px-3.5 py-2.5 focus:outline-none font-mono"
                    />
                  </div>
                  {mode === "withdraw" && (
                ...
                <button type="submit" disabled={submitting}
                  className="w-full bg-[#0d1520] text-white text-[11px] font-semibold uppercase tracking-[0.12em] py-3 rounded-xl hover:bg-[#1a2d4a] transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <button type="button" onClick={() => { setMode("idle"); setAmount(""); }}
                  className="w-full border border-[#E6E8EB] text-muted-foreground text-[11px] font-medium uppercase tracking-wider py-2.5 rounded-xl hover:bg-[#F5F6F7] transition-colors">
                  Cancel
                </button>
              </form>
            ) : (
        ...
        <div className={CARD}>
          ...
          <div className="p-6 space-y-6">
            <div>
              ...
              <div className="text-[32px] font-bold tracking-tight tabular-nums text-foreground">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={m.label} className="p-4 rounded-xl bg-[#F5F6F7]">
                  <div className="text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground mb-2">{m.label}</div>
                  <div className="text-[16px] font-semibold text-foreground tabular-nums font-mono">{m.value}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className={`${CARD} overflow-hidden`}>
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E6E8EB]">
          ...
          <button onClick={() => { refetchTx(); refetchBalance(); }}
            className="text-muted-foreground hover:text-foreground transition-colors p-2 rounded-full hover:bg-[#F5F6F7]">
                ...
                {txData?.transactions?.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-[#F5F6F7] transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-right font-semibold font-mono text-foreground text-[11px]">
